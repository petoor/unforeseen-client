...
# if the settings has a flags=inactive it will be invalid or incomplete.
# set exposure_auto to 1 in order to not screw with
# the framerate while recording
import subprocess
...
def apply_camera_settings(camera):
    ...
    with open(f"cameras/{camera.split('/')[-1]}.txt", "r") as f:
        ...
        for line in f:
            print(line)
            camera_settings[line.lstrip().split(" ")[0]] = int(
                line.lstrip().split("value=")[-1].split(" ")[0]
            )
