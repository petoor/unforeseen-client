...
class GstPipeline:
    ...
    def __init__(self, pipeline: str, width: int, height: int, framerate: int, buffer_length: int, record_path: str, model=None, url=None, token=None, bucket=None, org=None, in_pin=None, out_pin=None) -> None:
        self.running = False
        self.gstsample = None
        self.width = width
        self.height = height
    ...
    def run(self):
        self.running = True
        worker = threading.Thread(target=self.inference_record_loop)
        worker.start()
    ...
    def inference_record_loop(self) -> None:
...
if __name__=="__main__":
    ...
    parser.add_argument('--pipeline_path', default="analysis/pipelines/hello-world-raw.txt",
    ...
    GstPipeline(pipeline, width, height, framerate, framerate* 900, record_path, model, url, token, bucket, org, in_pin, out_pin).run()
